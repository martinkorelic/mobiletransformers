...
Here are some old results made from testing some of the different approaches.
...
![](./train_step_memory_tinyllama.png)
...
![](./inference_memory_tinyllama.png)
...
> **Solution**: I created a different kind of inference graph and saved it separately to the device, which has inputs set to accept updated weights from the checkpoint state memory. However, this creates more memory overhead as the weights need to be also copied to memory before they are transferred to GenAI inference graph, while the other frozen weights are discarded before to preserve memory.
>
> **Better solution**: I created my own native inference which will give me more control over the loading of the weights and inference / generation on-device. It's still compatible with ONNX GenAI graph, therefore I will still leave the solution for ONNX GenAI in the application.
...
> TODO: Create and add my own tokenizer implementation from `tokenizer.model` of the Huggingface format.
...
> **Solution**: This was solved with creating a separate inference graph, where we avoid creating a new inference graph from the training graph and instead pre-load the inference graph with the updated weights using the `SessionOptions`.

- **Loading and saving model adapters for training and inference**

> TODO: Need to create an implementation on-device that will save and load seperate adapter weights into training and inference:
- For training these weights need to be loaded seperately into TrainingSession with Checkpoint by using something like using [UpdateParameter](https://onnxruntime.ai/docs/api/c/class_ort_1_1_checkpoint_state.html#ae4cfae7bd39cb42969a63cc4e4276994).
- After training is done these updated parameters need to be saved and offloaded (IMPORTANT: Optimizer states + trained weights!! need to be saved and loaded)
> TODO: Can we extract optimizer states from the CheckpointState?
- When starting the inference model, we need to collect the adapter weights, use another graph to perform some operations of merging and then finally loading them into inference session
- Also it would be useful to cache the Inference adapter weights so we can avoid the overhead of additionally computing merging if there was no training performed beforehand
