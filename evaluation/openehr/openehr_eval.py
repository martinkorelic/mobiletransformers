...
llm_generator = GeminiModel(
    model_name="gemini-2.0-flash",
    api_key=GEMINI_API_KEY
)

llm_evaluator = GeminiModel(
...
for comparison_name, test_cases in all_test_cases.items():
    ...
    results = evaluate(
        ...
        metrics=[faithfulness_metric, clinical_quality_metric]
    )
    ...
    summary_stats = {
        ...
        "faithfulness": {
            ...
            "pass_rate": sum(1 for s in faithfulness_scores if s >= 0.9) / len(faithfulness_scores) if faithfulness_scores else 0,
            "scores": faithfulness_scores
        },
        "clinical_quality": {
            "average": sum(clinical_quality_scores) / len(clinical_quality_scores) if clinical_quality_scores else 0,
            "pass_rate": sum(1 for s in clinical_quality_scores if s >= 7.0) / len(clinical_quality_scores) if clinical_quality_scores else 0,
    ...
    # Print results
    print(f"Faithfulness - Average: {summary_stats['faithfulness']['average']:.3f}, Pass Rate: {summary_stats['faithfulness']['pass_rate']:.1%}")
    print(f"Clinical Quality - Average: {summary_stats['clinical_quality']['average']:.1f}, Pass Rate: {summary_stats['clinical_quality']['pass_rate']:.1%}")
...
for comparison_name, stats in evaluation_results.items():
    ...
    print(f"  Faithfulness: {stats['faithfulness']['average']:.3f} (Pass: {stats['faithfulness']['pass_rate']:.1%})")
    print(f"  Clinical Quality: {stats['clinical_quality']['average']:.1f} (Pass: {stats['clinical_quality']['pass_rate']:.1%})")
...
print(f"1. SLM Performance vs LLM:")
print(f"   Document Context - Faithfulness: {slm_llm_doc['faithfulness']['average']:.3f}, Quality: {slm_llm_doc['clinical_quality']['average']:.1f}")
print(f"   Chunked Context  - Faithfulness: {slm_llm_chunk['faithfulness']['average']:.3f}, Quality: {slm_llm_chunk['clinical_quality']['average']:.1f}")
...
print(f"   SLM: Document vs Chunked - Faithfulness: {doc_chunk_slm['faithfulness']['average']:.3f}, Quality: {doc_chunk_slm['clinical_quality']['average']:.1f}")
print(f"   LLM: Document vs Chunked - Faithfulness: {doc_chunk_llm['faithfulness']['average']:.3f}, Quality: {doc_chunk_llm['clinical_quality']['average']:.1f}")
