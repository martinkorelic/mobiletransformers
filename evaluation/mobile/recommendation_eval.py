...
from deepeval.test_case import LLMTestCase, LLMTestCaseParams, ArenaTestCase
from deepeval.models.base_model import DeepEvalBaseLLM
from langchain_openai import AzureChatOpenAI

# Import configuration from config module
from config import AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_API_KEY, AZURE_DEPLOYMENT_NAME, AZURE_MODEL_NAME, AZURE_API_VERSION
...
class RecommendationEvaluator:
    ...
    def __init__(self, 
        ...
        print(f"RecommendationEvaluator initialized with Azure OpenAI:")
    ...
    def load_data(self, base_json_path: str, finetuned_json_path: str) -> Tuple[List[Dict], List[Dict]]:
        ...
        with open(finetuned_json_path, 'r', encoding='utf-8') as f:
        ...
        if base_ids != finetuned_ids:
            print(f"Warning: Sample ID mismatch detected!")
    ...
    def evaluate_model(self, data: List[Dict], model_name: str) -> List[Dict]:
        ...
        print(f"  Running evaluation with Azure OpenAI...")
    ...
    def _calculate_comparison_stats(self, base_results: List[Dict], finetuned_results: List[Dict]) -> Dict[str, Any]:
        """
        ...
        for metric_name in available_metrics:
            try:
                ...
                finetuned_passed = sum(1 for result in finetuned_results if result['metrics'][metric_name]['success'])
        ...
        if metric_keys:
            ...
            stats['overall_summary'] = {
                ...
                'winner': 'Finetuned' if overall_finetuned > overall_base else 'Base' if overall_base > overall_finetuned else 'Tie'
            }
    ...
    def arena_comparison(self, base_json_path: str, finetuned_json_path: str) -> Dict[str, Any]:
        ...
        for i, (base_item, finetuned_item) in enumerate(zip(base_data, finetuned_data)):
            ...
            if base_item['sample_id'] != finetuned_item['sample_id']:
                print(f"Warning: Sample ID mismatch at index {i}: {base_item['sample_id']} vs {finetuned_item['sample_id']}")
                continue
        ...
        arena_summary = {
            ...
            'base_wins': base_wins,
            'finetuned_win_rate': round(finetuned_wins / total_comparisons, 3) if total_comparisons > 0 else 0,
            'base_win_rate': round(base_wins / total_comparisons, 3) if total_comparisons > 0 else 0,
            'overall_winner': 'Finetuned' if finetuned_wins > base_wins else 'Base' if base_wins > finetuned_wins else 'Tie'
        }
        
        print(f"\n✓ Arena comparison complete!")
        print(f"  Finetuned wins: {finetuned_wins}/{total_comparisons} ({arena_summary['finetuned_win_rate']:.1%})")
        ...
        return {
            ...
            'detailed_results': arena_results
        }
    ...
    def print_summary(self, results: Dict[str, Any]):
        ...
        for metric_name, metric_stats in stats.items():
            ...
            print(f"\n{metric_name}:")
            print(f"  Base Model      - Average: {metric_stats['base_average']:.3f}, Passed: {metric_stats['base_passed']}")
            print(f"  Finetuned Model - Average: {metric_stats['finetuned_average']:.3f}, Passed: {metric_stats['finetuned_passed']}")
            print(f"  Improvement     - {metric_stats['improvement']:+.3f} (Pass rate: {metric_stats['pass_rate_improvement']:+d})")
...
def main():
    ...
    try:
        ...
        finetuned_json_path = "experiment_results/train-qwen2-recommendation-mobile/finetuned_evaluation_outputs.json"
        output_path = f"comparison_evaluation_results.json"
