...
def create_quantization_utility_graph(weight_shape, axis=0, block_size=128):
    """Create a utility graph that quantizes float weights to int8."""
    ...
    quantized_output = helper.make_tensor_value_info("quantized_weight", TensorProto.INT8, weight_shape)
    
    # Create QuantizeLinear node - removed zero_point from inputs, added output_dtype
...
def compute_quantization_params(weight_data, axis=0, block_size=128):
    ...
    for block_idx in range(scale_shape[axis]):
        ...
        # For int8: range is [-128, 127] (256 values total)
