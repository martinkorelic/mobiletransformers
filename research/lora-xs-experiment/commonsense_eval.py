# Code based on https://github.com/NVlabs/DoRA script with minimal changes.
        ...
        for key, module in key_list:
            if isinstance(model.peft_config['default'].target_modules, str):
                ...
                target_module_found = any(key.endswith(target_key)
                                          for target_key in model.peft_config['default'].target_modules)
...
def load_data(args) -> list:
    ...
    """
    file_path = f'data/{args.dataset}_test.json'
...
def parse_args():
    ...
    parser.add_argument('--dataset',
                        ...
                        required=True)
    parser.add_argument('--model', choices=['LLaMA-7B', "LLaMA-13B", 'LLaMA2-7B', 'LLaMA3-8B'], required=False)
    parser.add_argument('--adapter', choices=['LoRAXS', 'LoRA'],
                        required=False)
...
def load_model(args) -> tuple:
    ...
    if lora_weights is None and args.adapter == 'LoRA':
        raise ValueError(f'can not find lora weight, the value is: {lora_weights}')

    load_8bit = args.load_8bit
    if args.model and "LLaMA" in args.model:
