...
class AblationLayer(BaseTunerLayer):
    ...
    def __init__(self, base_layer: nn.Module, ablation_config : AblationConfig, ablation_variant : AblationVariant, **kwargs) -> None:
    ...
    def update_layer(self, original_weights, adapter_name, ablation_config : AblationConfig, ablation_variant : AblationVariant):
        ...
        self.up_project[adapter_name] = nn.Parameter(
            torch.zeros(ablation_config.r, original_weights.out_features),
            requires_grad=True
        )
...
class Linear(nn.Module, AblationLayer):
    ...
    def __init__(self,
        ...
        self.update_layer(
            base_layer, adapter_name, ablation_config, ablation_variant
        )

        # Metric tracking setup
        self.metric_tracking = ablation_config.metric_tracking if hasattr(ablation_config, 'metric_tracking') else False
    ...
    def _track_layer_metrics_calibration(self, layer_name, input_tensor, output_tensor, param_tensor, active_adapter):
        ...
        with torch.no_grad():
            ...
            self.weight_magnitudes.append({
                'layer_name': layer_name,
                'weight_norm': weight_norm.cpu().detach().numpy()
            })
    ...
    def _store_channel_metrics(self, active_adapter):
        ...
        window_metrics = {
            'step_range': (self.step_counter - self.track_n, self.step_counter),
            'layers': {}
        }
