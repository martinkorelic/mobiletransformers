...
def print_pooling_summary(pooling_config):
    ...
    final_dim = (
        word_dim * len(active_modes) if isinstance(word_dim, int) and len(active_modes) > 0 else word_dim
    )
    ...
    if pooling_config is None:
        ...
        try:
            ...
            with open(transformer_config_path) as f:
                ...
                print(
                    f"Warning: No pooling config found for {model_id}. "
...
def add_pooling_operations(model, pooling_config):
    ...
    if pooling_config.get("pooling_mode_mean_sqrt_len_tokens", False):
        mean_sqrt_output = add_mean_sqrt_len_pooling(
            graph, "last_hidden_state", "attention_mask", nodes_to_add
        )
    ...
    embedding_output = helper.make_tensor_value_info(
        "embedding", TensorProto.FLOAT, ["batch_size", final_dim]
    )
...
def add_cls_pooling(graph, input_name, nodes_to_add):
    ...
    indices_tensor = helper.make_tensor(name="cls_indices", data_type=TensorProto.INT64, dims=[1], vals=[0])
...
def add_max_pooling(graph, input_name, attention_mask_name, nodes_to_add):
    ...
    neg_inf_tensor = helper.make_tensor(
        name="neg_inf_value_max", data_type=TensorProto.FLOAT, dims=[], vals=[-1e9]
    )
    ...
    one_tensor_max = helper.make_tensor(
        name="one_value_max", data_type=TensorProto.FLOAT, dims=[], vals=[1.0]
    )
    one_node_max = helper.make_node(
        "Constant", inputs=[], outputs=["one_const_max"], value=one_tensor_max, name="one_constant_max"
    )
    ...
    max_axes_node = helper.make_node(
        "Constant", inputs=[], outputs=["max_axes_const"], value=max_axes_tensor, name="max_axes_constant"
    )
...
def add_mean_sqrt_len_pooling(graph, input_name, attention_mask_name, nodes_to_add):
    ...
    sqrt_node = helper.make_node(
        "Sqrt", inputs=["sequence_lengths_sqrt"], outputs=["sqrt_sequence_lengths"], name="sqrt_lengths"
    )
