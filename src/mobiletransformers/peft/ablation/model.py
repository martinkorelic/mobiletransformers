...
class AblationModel(BaseTuner):
    ...
    def _create_and_replace(
        ...
        **kwargs,
        ...
        if isinstance(target, Linear):
            target.update_layer(target, adapter_name, ablation_config, self.ablation_variant)
            ...
            new_module = self._create_new_module(
                ablation_config, self.ablation_variant, adapter_name, target, **kwargs
            )
    ...
    def _shared_and_store_weights(self, new_module, ablation_config):
        ...
        down_project_shape = (
            f"{new_module.in_features}x{ablation_config.r}"  # Use a string key for ParameterDict
        )
