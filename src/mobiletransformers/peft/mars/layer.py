...
from mobiletransformers.peft.mars.matrices import create_orthogonal_matrices
...
class QuantizedBaseLayer(nn.Module):
    ...
    def _check_bnb_availability(self):
        ...
        try:
            ...
            if self.bits == 4:
                ...
                print(
                    f"BitsAndBytes doesn't support {self.bits}-bit quantization. Using manual implementation."
                )
    ...
    def _dequantize_weights_bnb(self):
        ...
        try:
            ...
            return torch.zeros((self.out_features, self.in_features), device=self.device, dtype=torch.float32)
...
class MarsLayer(BaseTunerLayer):
    ...
    def update_layer(self, original_weights, adapter_name, rank, alpha, projection_type, **kwargs):
        ...
        if self.quantize_base and self.preserve_errors:
            self._compute_error_residuals_and_svd(
                original_weights.weight.data.clone(), adapter_name, rank, self.shared_rank
            )
        ...
        # If base layer is not quantized and standalone adapter
            ...
            self.up_project[adapter_name].weight.data = torch.nn.init.zeros_(
                self.up_project[adapter_name].weight.data
            )
...
class SharedMLPAdapter(nn.Module):
    ...
    def _update_layer(self, u_proj=None, sigma=None, res_projection_type=None):
        ...
        if self.orth_init:
            orthogonal_matrices = create_orthogonal_matrices(
                self.rank, num_matrices=2, mean=0, std=1 / self.rank, initial_matrix=sigma, device="cuda"
            )
...
class SharedAttentionAdapter(nn.Module):
    ...
    def _update_layer(self, u_proj=None, sigma=None, res_projection_type=None):
        ...
        if self.orth_init:
            orthogonal_matrices = create_orthogonal_matrices(
                self.rank,
                num_matrices=self.num_enabled,
                mean=0,
                std=1 / self.rank,
                initial_matrix=sigma,
                device="cuda",
            )
