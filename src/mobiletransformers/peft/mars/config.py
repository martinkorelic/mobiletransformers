...
class MarsConfig(PeftConfig):
    ...
    shared_r: int | None = field(default=None, metadata={"help": "Shared rank attention dimension"})
    ...
    enabled_qkv: tuple[str, ...] | None = field(
        default=("q", "k", "v"),
        metadata={
            "help": "Which QKV projections to enable in the shared QKV adapter. Please select between 'q', 'k' and 'v'."
        },
    )
    enabled_mlp: bool = field(
        default=True,
        metadata={
            "help": "Set this to True if we should have a shared down_proj and gate_proj down projection layers."
        },
    )
    mixture: bool = field(
        default=False,
    ...
    modules_to_quantize: list[str] | None = field(
        default=None,
        metadata={
            "help": (
    ...
    modules_to_preserve_errors: list[str] | None = field(
        default=None,
        metadata={
            "help": (
    ...
    quant_n_bits: int = field(
        default=8,
        metadata={
            "help": "Quantization type (bits for quantized weights) for MARS. Can be either '8' or '4'."
        },
    )
    use_bnb: bool = field(
        default=True,
    ...
    bias: str = field(
        default="none", metadata={"help": "Bias type for Mars. Can be 'none', 'all' or 'mars_only'"}
    )
