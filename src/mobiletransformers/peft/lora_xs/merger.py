...
def main(args):
    ...
    # #6: dispatch on the typed enum, not the raw wire string (PEFTMethod fails closed on unknown).
    if PEFTMethod(args.peft_method) is PEFTMethod.LORA:
        return load_and_merge_lora_model(
            model_name_or_path=args.base_model, peft_model_path=args.adapter, save_directory=args.output_path
        )
    ...
    peft_model_weights = {}
    with safe_open(
        os.path.join(args.adapter, "adapter_model.safetensors"), framework="pt", device="cpu"
    ) as f:
        for key in f.keys():
