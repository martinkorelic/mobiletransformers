...
from mobiletransformers.config.constants import ARTIFACT_CONFIG, ARTIFACT_VALIDATOR_CONFIG, TRAIN_CONFIG
...
from mobiletransformers.utils.templating import create_chat_input
...
def validate_generation(
    ...
    **kwargs,
    ...
    if test_generation_config["type"] == "genai":
        ...
        with open(genai_config_path, encoding="utf-8") as infile:
    ...
    if test_generation:
        ...
        if test_generation_config["hf_tokenizer"]:
            if tokenizer.chat_template is not None:
                messages = [{"role": "user", "content": test_generation_config["prompt"]}]
                test_generation_config["prompt"] = tokenizer.apply_chat_template(
            ...
            if "chat_template" in tokenizer_config:
                test_generation_config["prompt"] = create_chat_input(
                    test_generation_config["prompt"], tokenizer_config
                )
...
class MobileTransformerGenerator:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print("[INFO] MobileTransformerGenerator initialized successfully")
    ...
    def _initialize_tokenizer(self):
        ...
        tokenizer = AutoTokenizer.from_pretrained(self.model_id, token=os.environ.get("HF_TOKEN"))
    ...
    def update_generation_config(self, new_config):
        ...
        print("[INFO] Generation configuration updated")
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Validator for exported ONNX artifacts for on-device inference.",
    ...
    parser.add_argument("--inference_artifact_dir", type=str, help="Path to inference artifact directory.")
    parser.add_argument("--inference_artifact_name", type=str, help="Name of the inference artifact model.")
    parser.add_argument(
        "--test_generation",
        type=bool,
        default=True,
    ...
    if args.config_file:
        ...
        args.inference_artifact_dir = os.path.join(config_dict[ARTIFACT_CONFIG]["build_path"], "inference")
        args.inference_artifact_name = (
            f"{config_dict[ARTIFACT_CONFIG]['inference_export_config']['output_inference_model']}.onnx"
        )

        extra_args["tokenizer_dir"] = os.path.join(config_dict[ARTIFACT_CONFIG]["build_path"], "tokenizer")
        extra_args["temp_weights_dir"] = os.path.join(
            config_dict[ARTIFACT_CONFIG]["build_path"], "train", "temp_weights"
        )
