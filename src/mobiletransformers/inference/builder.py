...
# ORT renamed this module/class when the quantizer was generalised to N-bit, deleting the old
# names outright. Resolved at call time so BOTH ORT lines this repo pins keep working; this one
# import is what made the whole file unimportable, blocking Migration S6. See quantizer_compat.
from mobiletransformers.export.quantizer_compat import load_weight_only_matmul_quantizer

MatMul4BitsQuantizer = load_weight_only_matmul_quantizer()
import argparse
import gc
import json
import os
...
class Model:
    def __init__(self, config, io_dtype, onnx_dtype, ep, cache_dir, extra_options):
        self.context_length = (
            config.seq_length if hasattr(config, "seq_length") else config.max_position_embeddings
        )
        ...
        self.quant_type = (
            config.quantization_config["quant_method"] if hasattr(config, "quantization_config") else None
        )
        ...
        if hasattr(config, "rope_scaling") and config.rope_scaling is not None:
            if "short_factor" in config.rope_scaling:
                ...
                short_mscale = (
                    config.rope_scaling["short_mscale"] if "short_mscale" in config.rope_scaling else 0
                )
                long_mscale = (
                    config.rope_scaling["long_mscale"] if "long_mscale" in config.rope_scaling else 0
                )
                short_mscale = (
                    short_mscale
                    if short_mscale > 0
                ...
                low_freq_factor = (
                    config.rope_scaling["low_freq_factor"] if "low_freq_factor" in config.rope_scaling else 0
                )
                high_freq_factor = (
        ...
        softcap = (
            config.attn_logit_softcapping if hasattr(config, "attn_logit_softcapping") else 0.0
        ...
        local_blocks = (
            config.blocksparse_num_local_blocks if hasattr(config, "blocksparse_num_local_blocks") else 0
        )
        vert_block_stride = (
            config.blocksparse_vert_stride if hasattr(config, "blocksparse_vert_stride") else 0
        )
        homo_head = (
            config.blocksparse_homo_head_pattern
            if hasattr(config, "blocksparse_homo_head_pattern")
            else False
        )
        self.attention_attrs = {
            ...
            "force_unpacked_matmul": extra_options[
                "force_unpacked_matmul"
            ],  # Force the matmul of attention to be unpacked (for importing LoRA merged adapters)
        ...
        self.quant_attrs = {
            "int4": {
                "accuracy_level": int(
                    extra_options.get("int4_accuracy_level", 0)
                ),  # Default is 0 for non-QDQ formats, default is 4 for QDQ formats
    ...
    def make_genai_config(self, model_name_or_path, extra_kwargs, out_dir):
        try:
            config = GenerationConfig.from_pretrained(
                model_name_or_path, token=self.hf_token, trust_remote_code=True, **extra_kwargs
            )
        except:
        ...
        genai_config = {
            "model": {
                ...
                else 1,  # config.bos_token_id not present in ChatGLM model configs.
            ...
            "search": {
                ...
                if hasattr(config, "diversity_penalty")
                else 0.0,
    ...
    def _get_prompt_templates(self, hf_name, extra_kwargs):
        try:
            ...
            tokenizer = AutoTokenizer.from_pretrained(
                hf_name, token=self.hf_token, trust_remote_code=True, eos_token=None, **extra_kwargs
            )
            system_template = tokenizer.apply_chat_template(
                [{"role": "system", "content": "{Content}"}], tokenize=False
            )
            system_user_template = tokenizer.apply_chat_template(
                [{"role": "system", "content": "{Content}"}, {"role": "user", "content": "{Content}"}],
                tokenize=False,
            )
            system_user_assistant_template = tokenizer.apply_chat_template(
                [
            ...
            assert system_user_template.startswith(system_template), (
                "Chat templates may contain padding tokens, leading to incorrect prompt templates"
            )
            assert system_user_assistant_template.startswith(system_user_template), (
                "Chat templates may contain padding tokens, leading to incorrect prompt templates"
            )
    ...
    def save_model(self, out_dir):
        ...
        model = helper.make_model(
            opset_imports=[
                self.clear_field(
                    helper.make_operatorsetid("", 21 if self.quant_attrs["use_qdq"] else 14), "domain"
                ),
    ...
    def to_int4(self, model, dynamic_quantize=False):
        ...
        if self.quant_attrs["int4"]["non_quant_nodes_keywords"]:
        ...
        quant = MatMul4BitsQuantizer(
            ...
            # Exclude merged LoRA layers from quantization
            nodes_to_exclude=excluding,
        ...
        for i in range(self.num_layers):
            ...
            inputs.append(
                helper.make_tensor_value_info(
                    key_name,
                    self.input_types["past_key_values.key"],
            ...
            inputs.append(
                helper.make_tensor_value_info(
                    value_name,
                    self.input_types["past_key_values.value"],
            ...
            outputs.append(
                helper.make_tensor_value_info(
                    key_name, self.output_types["present.key"], shape=self.output_shapes["present.key"]
                )
            )
            value_name = f"present.{i}.value"
            outputs.append(
                helper.make_tensor_value_info(
                    value_name, self.output_types["present.value"], shape=self.output_shapes["present.value"]
                )
            )
    ...
    def make_constant(self, name):
        ...
        value = numpy_helper.from_array(
            np.array(num if dims == "0D" else list(num) if type(num) == tuple else [num], dtype=np_dtype),
            name=name.replace("constants", "numpy_helper"),
        )
    ...
    def make_matmul_fp16_or_fp32(self, matmul, name, root_input, **kwargs):
        ...
        if self.quant_attrs["force_transpose_inputs"]:
            ...
            self.make_external_tensor(
                matmul.weight.detach().numpy().astype(self.to_numpy_dtype[self.io_dtype]), weight
            )
            ...
            transposed_weight_shape = [
                original_weight_shape[1],
                original_weight_shape[0],
            ...
            self.make_node(
                "MatMul", inputs=[root_input, transposed_weight_output], outputs=[output], name=name
            )
            ...
            self.make_external_tensor(
                matmul.weight.detach().numpy().transpose().astype(self.to_numpy_dtype[self.io_dtype]), weight
            )
    ...
    def make_matmul_int4(self, matmul, basename, root_input, **kwargs):
        ...
        self.make_external_tensor(
            matmul.scales.detach().numpy().astype(self.to_numpy_dtype[self.io_dtype]), scales
        )
        ...
        self.make_value_info(
            output, self.io_dtype, shape=["batch_size", "sequence_length", matmul.out_features]
        )
    ...
    def make_dequantize_linear(self, dequantize_name, quantized_op):
        ...
        qweight_npy = qweight_npy.reshape(
            *qweight_npy.shape[:-2], qweight_npy.shape[-2] * qweight_npy.shape[-1]
        )
        ...
        scales_npy = scales_npy.reshape(
            *qweight_npy.shape[:-1], qweight_npy.shape[-1] * 2 // quantized_op.group_size
        )
        ...
        if hasattr(quantized_op, "qzeros") and quantized_op.qzeros is not None:
            ...
            zeros_npy = zeros_npy.reshape(
                *qweight_npy.shape[:-1], qweight_npy.shape[-1] // quantized_op.group_size
            )
        ...
        self.make_value_info(
            dequantize_output,
            self.io_dtype,
            shape=[*scales_npy.shape[:-1], scales_npy.shape[-1] * quantized_op.group_size],
        )
    ...
    def make_matmul_int4_qdq(self, matmul, matmul_name, root_input, **kwargs):
        ...
        self.make_value_info(
            matmul_output, self.io_dtype, shape=["batch_size", "sequence_length", matmul.out_features]
        )

        return matmul_name

    # TODO: Add own implementation of LoRA implementation on inference graph
    ...
    def make_packed_matmul(self, q_matmul, k_matmul, v_matmul, basename, root_input, **kwargs):
        if self.onnx_dtype in {"fp16", "fp32"}:
            return self.make_packed_matmul_fp16_or_fp32(
    ...
    def make_packed_matmul_fp16_or_fp32(self, q_matmul, k_matmul, v_matmul, name, root_input, **kwargs):
        ...
        class PackedMatMul:
            def __init__(self):
                self.weight = torch.concatenate(
                    [
                        q_matmul.weight.detach().cpu(),
                        k_matmul.weight.detach().cpu(),
                        v_matmul.weight.detach().cpu(),
                    ],
                    dim=0,
                ).reshape(N_q + N_kv + N_kv, H)
    ...
    def make_packed_matmul_int4(self, q_matmul, k_matmul, v_matmul, basename, root_input, **kwargs):
        ...
        class PackedMatMul:
            def __init__(self):
                self.qweight = torch.concatenate(
                    [
                        q_matmul.qweight.detach().cpu(),
                        k_matmul.qweight.detach().cpu(),
                        v_matmul.qweight.detach().cpu(),
                    ],
                    dim=0,
                )
                self.scales = torch.concatenate(
                    [
                        q_matmul.scales.detach().cpu(),
                        k_matmul.scales.detach().cpu(),
                        v_matmul.scales.detach().cpu(),
                    ],
                    dim=0,
                )
                self.qzeros = torch.concatenate(
                    [
                        q_matmul.qzeros.detach().cpu(),
                        k_matmul.qzeros.detach().cpu(),
                        v_matmul.qzeros.detach().cpu(),
                    ],
                    dim=0,
                )
    ...
    def make_embedding(self, embedding):
        ...
        self.make_value_info(
            gather_output, self.io_dtype, shape=["batch_size", "sequence_length", self.hidden_size]
        )
        ...
        if self.embed_attrs["scale"] != 1:
            ...
            mul_inputs = [
                gather_output,
                f"/model/constants/{self.to_str_dtype[self.io_dtype]}/0D/{self.embed_attrs['scale']}",
            ]
            mul_output = f"{mul_name}/output_0"
            self.make_node("Mul", inputs=mul_inputs, outputs=[mul_output], name=mul_name)
            self.make_value_info(
                mul_output, self.io_dtype, shape=["batch_size", "sequence_length", self.hidden_size]
            )
    ...
    def make_layernorm(self, layer_id, layernorm, skip, simple, location):
        ...
        self.make_external_tensor(
            layernorm.weight.detach().numpy().astype(self.to_numpy_dtype[self.io_dtype])
            + self.layernorm_attrs["add_offset"],
            weight,
        )
        bias = f"model.layers.{layer_id}.{location}_layernorm.bias"
        if not simple:
            self.make_external_tensor(
                layernorm.bias.detach().numpy().astype(self.to_numpy_dtype[self.io_dtype]), bias
            )
        ...
        self.make_value_info(
            output_0, self.io_dtype, shape=["batch_size", "sequence_length", self.hidden_size]
        )
        if skip and not self.layernorm_attrs["last_layernorm"]:
            self.make_value_info(
                output_3, self.io_dtype, shape=["batch_size", "sequence_length", self.hidden_size]
            )
    ...
    def make_rotary_embedding_caches_from_scratch(self):
        ...
        inv_freq = 1.0 / (
            self.rotemb_attrs["rescale_factors"]
            * (self.rotemb_attrs["theta"] ** (torch.arange(0, dim, 2, dtype=torch.int64).float() / dim))
        )
        if "rescale_inv_freq" in self.rotemb_attrs:
            inv_freq = self.make_inv_freq_rescaled(inv_freq)

        position_scale = (
            self.rotemb_attrs["position_scale"] if self.context_length == self.original_context_length else 1
        )
        t = (
            torch.arange(self.rotemb_attrs["cache_length"], dtype=self.rotemb_attrs["t_dtype"])
            * position_scale
        ).type_as(inv_freq)
        ...
        cos_cache, sin_cache = (
            emb.cos() * self.rotemb_attrs["mscale"],
            emb.sin() * self.rotemb_attrs["mscale"],
        )
    ...
    def make_rotary_embedding(self, rotemb, name, root_input, **kwargs):
        ...
        self.make_node(
            ...
            name=name,
            domain="com.microsoft",
            interleaved=self.rotemb_attrs["interleaved"],
            **kwargs,
        )
        self.make_value_info(
            output,
            ...
            shape=[
                "batch_size",
                "sequence_length",
                self.head_size * (self.num_kv_heads if "k_rotary" in name else self.num_attn_heads),
            ],
        )
    ...
    def make_rotary_embedding_multi_cache(self):
        ...
        if self.ep == "dml":
            ...
            self.make_value_info(
                if_cos_cache_output, self.io_dtype, shape=["max_sequence_length", "head_dim / 2"]
            )
            self.make_value_info(
                if_sin_cache_output, self.io_dtype, shape=["max_sequence_length", "head_dim / 2"]
            )
            return
        ...
        cos_cache_large, sin_cache_large = self.make_rotary_embedding_caches(
            rotemb, cos_cache_name=cos_cache_large_name, sin_cache_name=sin_cache_large_name
        )
        ...
        cos_cache_small, sin_cache_small = self.make_rotary_embedding_caches(
            rotemb, cos_cache_name=cos_cache_small_name, sin_cache_name=sin_cache_small_name
        )
        ...
        greater_inputs = [
            ...
            f"/model/constants/TensorProto.INT64/0D/{self.original_context_length}",
        ]
        ...
        self.make_node(
            ...
            then_branch=self.make_graph(
                ...
                outputs=[
                    helper.make_tensor_value_info(
                        cos_cache_large_name, self.io_dtype, shape=cos_cache_large.shape
                    ),
                    helper.make_tensor_value_info(
                        sin_cache_large_name, self.io_dtype, shape=sin_cache_large.shape
                    ),
                ],
                initializer=[],
                value_info=[],
                nodes=[
                    helper.make_node(
                        "Constant",
                        inputs=[],
                        outputs=[cos_cache_large_name],
                        name="/large/cos_cache/Constant",
                        value=numpy_helper.from_array(cos_cache_large),
                    ),
                    helper.make_node(
                        "Constant",
                        inputs=[],
                        outputs=[sin_cache_large_name],
                        name="/large/sin_cache/Constant",
                        value=numpy_helper.from_array(sin_cache_large),
                    ),
                ],
            ),
            else_branch=self.make_graph(
                ...
                outputs=[
                    helper.make_tensor_value_info(
                        cos_cache_small_name, self.io_dtype, shape=cos_cache_small.shape
                    ),
                    helper.make_tensor_value_info(
                        sin_cache_small_name, self.io_dtype, shape=sin_cache_small.shape
                    ),
                ],
                initializer=[],
                value_info=[],
                nodes=[
                    helper.make_node(
                        "Constant",
                        inputs=[],
                        outputs=[cos_cache_small_name],
                        name="/small/cos_cache/Constant",
                        value=numpy_helper.from_array(cos_cache_small),
                    ),
                    helper.make_node(
                        "Constant",
                        inputs=[],
                        outputs=[sin_cache_small_name],
    ...
    def make_repeat_kv(self, layer_id, root_input, past_kv, present_kv, **kwargs):
        ...
        #                 Reshape
        basename = (
            f"/model/layers.{layer_id}/attn/{'k_proj' if past_kv.endswith('key') else 'v_proj'}/repeat_kv"
        )
        ...
        reshape_1_inputs = [
            root_input,
            f"/model/constants/TensorProto.INT64/1D/0, 0, {self.num_kv_heads}, -1",
        ]
        self.make_reshape(
            reshape_1_name,
            reshape_1_inputs,
        ...
        concat_2_inputs = [
            ...
            f"/model/constants/TensorProto.INT64/1D/{self.num_attn_heads // self.num_kv_heads}",
        ...
        mul_1_inputs = [
            f"{unsqueeze_2_name}/output_0",
            f"/model/constants/TensorProto.INT64/0D/{self.num_attn_heads // self.num_kv_heads}",
        ]
        ...
        reshape_4_inputs = [
            ...
            f"/model/constants/TensorProto.INT64/1D/0, 0, {self.num_attn_heads * self.head_size}",
        ]
        self.make_reshape(
            reshape_4_name,
            reshape_4_inputs,
    ...
    def make_group_query_attention(self, name, **kwargs):
        ...
        self.make_node(
            ...
            scale=self.attention_attrs[
                "scale"
            ],  # local_window_size=self.window_size,  # Disable sliding window attribute temporarily
            softcap=self.attention_attrs["softcap"],
            do_rotary=self.attention_attrs["use_rotemb_in_attn"],
    ...
    def make_attention(self, layer_id, attention, root_input, **kwargs):
        ...
        if all_bias_exists and self.attention_attrs["use_packed_matmul"]:
            ...
            self.make_packed_add(
                attention.q_proj.bias.detach().numpy(),
                attention.k_proj.bias.detach().numpy(),
                attention.v_proj.bias.detach().numpy(),
                qkv_add_name,
        ...
        if (
            self.num_attn_heads != self.num_kv_heads
            and self.attention_attrs["op_type"] == "MultiHeadAttention"
        ):
    ...
    def make_attention_unpacked_lora(self, layer_id, attention, qkv_linear, root_input, **kwargs):
        ...
        q_proj.bias = (
            ...
            else torch.nn.Parameter(qkv_linear.bias[:q_size], requires_grad=False)
        )
        ...
        q_lora_B.bias = (
            None if lora_B.bias is None else torch.nn.Parameter(lora_B.bias[:q_size], requires_grad=False)
        )
        ...
        v_lora_B.bias = (
            None
            if lora_B.bias is None
            else torch.nn.Parameter(lora_B.bias[q_size + kv_size :], requires_grad=False)
        )
    ...
    def make_attention_unpacked_regular(self, layer_id, attention, qkv_linear, root_input, **kwargs):
        ...
        attention.v_proj.weight = torch.nn.Parameter(
            qkv_linear.weight[q_size + kv_size :, :], requires_grad=False
        )
    ...
    def make_mlp_unpacked_lora(self, layer_id, mlp, root_input):
        ...
        gate_proj.weight = torch.nn.Parameter(
            mlp.gate_up_proj.weight[: self.intermediate_size, :], requires_grad=False
        )
        ...
        gate_proj_lora_B.weight = torch.nn.Parameter(
            lora_B.weight[: self.intermediate_size, :], requires_grad=False
        )

        up_proj_lora_B = torch.nn.Linear(in_features=self.hidden_size, out_features=self.intermediate_size)
        up_proj_lora_B.weight = torch.nn.Parameter(
            lora_B.weight[self.intermediate_size :, :], requires_grad=False
        )

        # Create GateProj/UpProj LoRA layers.
        #
        # These wrapped `q_proj`/`k_proj` — names that are never bound in this method. Copy-paste from
        # `make_attention_unpacked_lora`, where `q_proj`/`k_proj` ARE the locals being wrapped. Here the
        # bases are `gate_proj`/`up_proj`, built above and otherwise completely unused, which is what
        # makes the intent unambiguous. As written this raised NameError the moment an unpacked-MLP LoRA
        # model was exported; ruff F821 caught it when the file entered the lint gate in S6.
    ...
    def make_mlp_fc(self, layer_id, mlp, root_input):
        ...
        self.make_add_bias(
            mlp.fc1.bias.detach().numpy(), fc1_add_name, root_input=f"{fc1_matmul_name}/output_0"
        )
        ...
        self.make_add_bias(
            mlp.fc2.bias.detach().numpy(), fc2_add_name, root_input=f"{fc2_matmul_name}/output_0"
        )
    ...
    def make_block_sparse_moe(self, layer_id, bsm, root_input):
        ...
        self.make_gather(
            gather_name, [f"{shape_name}/output_0", "/model/constants/TensorProto.INT64/0D/2"], axis=0
        )

        unsqueeze_name = f"{gate_ops_base}/Unsqueeze"
        self.make_unsqueeze(
            unsqueeze_name,
            [f"{gather_name}/output_0", "/model/constants/TensorProto.INT64/1D/0"],
            dtype=TensorProto.INT64,
            shape=[1],
        )
        ...
        self.make_concat(
            concat_name,
            ["/model/constants/TensorProto.INT64/1D/-1", f"{unsqueeze_name}/output_0"],
            dtype=TensorProto.INT64,
            shape=[2],
            axis=0,
        )
        ...
        def quant_dequant(weights, quant_mode: bool = True):
            ...
            try:
                ...
                raise RuntimeError(
                    "tensorrt_llm is needed to use torch.ops.trtllm._symmetric_quantize_last_axis_of_batched_matrix()"
                )
        ...
        inputs = [
            ...
            moe_expert_weight_1_name,
            moe_expert_scales_1_name,
            bias_ph,
            moe_expert_weight_2_name,
            moe_expert_scales_2_name,
            bias_ph,
            moe_expert_weight_3_name,
            moe_expert_scales_3_name,
        ]
        ...
        self.make_node(
            ...
            expert_weight_bits=(4 if use_int4 else 8),
        )
    ...
    def make_gelu(self, layer_id, root_input, activation):
        ...
        self.make_node(
            activation, inputs=[root_input], outputs=[output], name=gelu_name, domain="com.microsoft"
        )
        self.make_value_info(
            output, self.io_dtype, shape=["batch_size", "sequence_length", self.intermediate_size]
        )
    ...
    def make_activation(self, layer_id, root_input):
        if self.activation in {"silu", "swish", "swiglu"}:
            output_name = self.make_activation_with_mul(
                layer_id, root_input, activation="Sigmoid", domain=None
            )
    ...
    def make_lm_head(self, lm_head):
        ...
        matmul_name = self.make_matmul(
            lm_head, matmul_basename, root_input, logits=not bias_exists and not scale_exists
        )

        if bias_exists:
            add_name = "/lm_head/Add"
            self.make_add_bias(
                lm_head.bias.detach().numpy(),
                add_name,
                root_input=f"{matmul_name}/output_0",
                logits=not scale_exists,
            )

        if scale_exists:
            mul_name = "/lm_head/Mul"
            mul_inputs = [
                f"{matmul_name if not bias_exists else add_name}/output_0",
            ...
            self.make_node("Mul", inputs=mul_inputs, outputs=[mul_output], name=mul_name)
            self.make_value_info(
                mul_output, self.io_dtype, shape=["batch_size", "sequence_length", self.vocab_size]
            )
        ...
        if mask_exists:
            ...
            where_inputs = [
                logits_mask_name,
                f"/model/constants/{self.to_str_dtype[self.io_dtype]}/0D/{np.finfo(self.to_numpy_dtype[self.io_dtype]).min}",
                f"{mul_name}/output_0",
            ]
            where_output = "logits"
            self.make_node("Where", inputs=where_inputs, outputs=[where_output], name=where_name)
            self.make_value_info(
                where_output, self.io_dtype, shape=["batch_size", "sequence_length", self.vocab_size]
            )
    ...
    def make_model(self, input_path):
        ...
        if input_path.endswith(".gguf"):
            ...
            # Load PyTorch model
            extra_kwargs = (
                {"num_hidden_layers": self.num_layers} if "num_hidden_layers" in self.extra_options else {}
            )
        ...
        if "adapter_path" in self.extra_options:
            ...
            if hasattr(model, "peft_config"):
                self.lora_layers_keywords = ["lora"]
                self.lora_layers_keywords = model.peft_config["default"].target_modules
    ...
    def make_attention_mask_reformatting_for_mha(self):
        ...
        shared_unsqueeze_name, end_expand_name = self.make_input_ids_subgraph(
            input_ids_basename, past_key_gather_name
        )
        ...
        tile_inputs = [
            ...
            f"/model/constants/TensorProto.INT64/1D/1, {self.num_attn_heads}, 1, 1",
        ]
    ...
    def make_input_ids_subgraph(self, basename, past_key_gather_name):
        ...
        unsqueeze_3_name = (
            f"{basename}/Unsqueeze_3"  # shared unsqueeze for input_ids and past_key_values.0.key
        )
        ...
        constant_shape_value = numpy_helper.from_array(
            np.array([np.finfo(constant_shape_numpy_dtype).min], dtype=constant_shape_numpy_dtype)
        )
        self.make_constant_of_shape(
            constant_shape_name,
    ...
    def make_attention_mask_subgraph(self, basename, unsqueeze_for_concat):
        ...
        self.make_unsqueeze(
            unsqueeze_3_name, unsqueeze_3_inputs, dtype=TensorProto.INT64, shape=attention_mask_shape
        )
        unsqueeze_4_name = f"{basename}/Unsqueeze_4"
        ...
        self.make_unsqueeze(
            unsqueeze_4_name, unsqueeze_4_inputs, dtype=TensorProto.INT64, shape=attention_mask_shape
        )
        ...
        self.make_cast(
            cast_1_name, f"{expand_name}/output_0", dtype=self.io_dtype, shape=["unk", "unk", "unk", "unk"]
        )
        ...
        self.make_cast(
            cast_2_name, f"{sub_name}/output_0", dtype=TensorProto.BOOL, shape=["unk", "unk", "unk", "unk"]
        )
        where_2_name = f"{basename}/Where_2"
        where_2_inputs = [
    ...
    def make_common_mask_reformat_subgraph(
        ...
        shape_1_name = f"{basename}/Shape_1"
        self.make_shape(
            shape_1_name, root_input, shape=[3] if self.exclude_embeds and input_ids_subgraph else [2]
        )
        shape_2_name = f"{basename}/Shape_2"
        self.make_shape(
            shape_2_name, root_input, shape=[3] if self.exclude_embeds and input_ids_subgraph else [2]
        )
        gather_1_name = f"{basename}/Gather_1"
        gather_1_inputs = [f"{shape_1_name}/output_0", "/model/constants/TensorProto.INT64/0D/0"]
        self.make_gather(gather_1_name, gather_1_inputs, axis=0)
    ...
    def make_attention_mask_reformatting_for_gqa(self):
        ...
        self.make_reduce_sum(
            reduce_sum_name, reduce_sum_inputs, dtype=TensorProto.INT64, shape=["batch_size", 1]
        )
    ...
    def make_attention_mask_reformatting_for_sparse_attn(self):
        ...
        self.make_cast(
            cast_1_name, f"{reduce_sum_name}/output_0", dtype=TensorProto.INT32, shape=["batch_size", 1]
        )
    ...
    def make_position_ids_reformatting(self):
        ...
        self.make_shape(
            shape_name,
            root_input="input_ids" if not self.exclude_embeds else "inputs_embeds",
            shape=[2] if not self.exclude_embeds else [3],
        )
        gather_name = f"{basename}/Gather"
        gather_inputs = [f"{shape_name}/output_0", "/model/constants/TensorProto.INT64/0D/1"]
        self.make_gather(gather_name, gather_inputs, axis=0)
...
class MistralModel(Model):
    def __init__(self, config, io_dtype, onnx_dtype, ep, cache_dir, extra_options):
        ...
        self.position_ids_name = (
            f"{self.make_position_ids_reformatting()}/output_0"
            if not self.attention_attrs["use_rotemb_in_attn"]
            else "position_ids"
        )
...
class PhiModel(Model):
    def __init__(self, config, io_dtype, onnx_dtype, ep, cache_dir, extra_options):
        ...
        self.rotemb_attrs["rotary_embedding_dim"] = int(
            self.head_size * self.rotemb_attrs["partial_rotary_factor"]
        )
        self.mlp_attrs["use_proj"], self.mlp_attrs["use_fc"] = False, True

    def make_rotary_embedding(self, rotemb, name, root_input, **kwargs):
        super().make_rotary_embedding(
            rotemb,
            name,
            root_input,
            num_heads=self.rotemb_attrs["num_heads"],
...
class Phi3Mini128KModel(Phi3Mini4KModel):
    ...
    def make_position_ids_reformatting(self):
        ...
        self.make_add(
            add_1_name, add_1_inputs, dtype=TensorProto.INT64, shape=["batch_size", "sequence_length"]
        )
...
class Phi3Small8KModel(Model):
    ...
    def make_mlp_proj(self, layer_id, mlp, root_input):
        ...
        slice_1_inputs = [
            ...
            f"/model/constants/TensorProto.INT64/1D/{np.iinfo(np.int64).max}",
            "/model/constants/TensorProto.INT64/1D/-1",
            "/model/constants/TensorProto.INT64/1D/2",
        ]
        self.make_slice(
            slice_1_name,
            slice_1_inputs,
        ...
        clip_1_inputs = [
            f"{slice_1_name}/output_0",
            "",
            f"/model/constants/{self.to_str_dtype[self.io_dtype]}/0D/{self.clamp_limit}",
        ]
        self.make_clip(
            clip_1_name,
            clip_1_inputs,
...
class Phi3MoE128KModel(MistralModel):
    def __init__(self, config, io_dtype, onnx_dtype, ep, cache_dir, extra_options):
        ...
        self.moe_attrs["use_int4"] = (
            0 if "use_8bits_moe" in extra_options and extra_options["use_8bits_moe"] == "1" else 1
        )
    ...
    def make_layer(self, layer_id, layer):
        ...
        self.make_block_sparse_moe(
            layer_id, layer.block_sparse_moe, root_input=self.layernorm_attrs["output_0"]
        )
...
class ChatGLMModel(Model):
    def __init__(self, config, io_dtype, onnx_dtype, ep, cache_dir, extra_options):
        super().__init__(config, io_dtype, onnx_dtype, ep, cache_dir, extra_options)
        self.rotemb_attrs["num_heads"] = self.num_attn_heads
        self.rotemb_attrs["partial_rotary_factor"] = (
...
def check_extra_options(kv_pairs):
    ...
    if "non_quant_nodes" in kv_pairs:
        op_types_to_quantize = ()
        for op_type in kv_pairs["non_quant_nodes"].split("/"):
            op_types_to_quantize += (op_type,)
        kv_pairs["non_quant_nodes"] = op_types_to_quantize

    if "use_8bits_moe" in kv_pairs:
        assert kv_pairs["use_8bits_moe"] == "1" or kv_pairs["use_8bits_moe"] == "0", (
            "use_8bits_moe must be 0 or 1."
        )
    ...
    if "enable_cuda_graph" in kv_pairs:
        assert kv_pairs["enable_cuda_graph"] == "1" or kv_pairs["enable_cuda_graph"] == "0", (
            "enable_cuda_graph must be 0 or 1."
        )
...
def create_model(
    ...
    if "adapter_path" in extra_options:
        ...
        peft_config = PeftConfig.from_pretrained(
            extra_options["adapter_path"], token=hf_token, trust_remote_code=True, **extra_kwargs
        )
    ...
    io_dtype = (
        TensorProto.FLOAT
        if precision in {"int8", "fp32"} or (precision == "int4" and execution_provider == "cpu")
        else TensorProto.FLOAT16
    )

    if "config_only" not in extra_options:
        # #6: the 14-branch architecture ladder that used to live here is now DATA in
        # config/registry/architecture.py. Three of those branches did more than pick a class —
        # PhiMoE forced cuda+int4, Phi3V forced exclude_embeds, ChatGLM forced hidden_act=swiglu —
        # so the registry row carries those as option_overrides / extra_option_overrides /
        # config_overrides rather than losing them. Adding an architecture is a row, not an elif.
        ...
        try:
            ...
            raise NotImplementedError(f"The {hf_name} model is not currently supported.") from exc

        for message in spec.warnings:
            print(f"WARNING: {message}")
        # Forced export options (e.g. PhiMoE's MoE op is CUDA-only and quantized-only in ORT).
        execution_provider = spec.option_overrides.get("execution_provider", execution_provider)
        precision = spec.option_overrides.get("precision", precision)
        for key, value in spec.extra_option_overrides.items():
            extra_options[key] = value
        # Forced HF-config attributes, applied before the builder reads them.
        for key, value in spec.config_overrides.items():
            setattr(config, key, value)

        variant_value = getattr(config, spec.variant_key, None) if spec.variant_key else None
        try:
            model_class = spec.load_inference_model_class(variant_value)
        except UnsupportedModelError as exc:
            raise NotImplementedError(f"The {hf_name} model is not currently supported.") from exc

        onnx_model = model_class(config, io_dtype, precision, execution_provider, cache_dir, extra_options)
...
if __name__ == "__main__":
    ...
    if args.config:
        ...
        # Specific
        args.model_name = config_dict[TRAIN_CONFIG]["model_id"]
        args.cache_dir = os.environ["HF_CACHE"]
        ...
        extra_options["int4_accuracy_level"] = (
            config_dict[INFERENCE_CONFIG]["int4_accuracy_level"]
            if "int4_accuracy_level" in config_dict[INFERENCE_CONFIG]
            else 4
        )
        extra_options["int4_block_size"] = (
            config_dict[INFERENCE_CONFIG]["int4_block_size"]
            if "int4_block_size" in config_dict[INFERENCE_CONFIG]
            else 32
        )
