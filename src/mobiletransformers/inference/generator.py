...
def generate_tokens_onnx(
    ...
    output_name="logits",
    max_length=100,
    sampling={"method": "greedy", "temperature": 1.0, "topP": 0.9, "topK": 50},
    decode_between=True,
    **kwargs,
):
    """
    ...
    if with_past:
        ...
        for i in range(config.num_hidden_layers):
            past_key_values[f"past_key_values.{i}.key"] = np.random.rand(
                *(1, num_kv_heads, 0, head_size)
            ).astype(np.float32)
            past_key_values[f"past_key_values.{i}.value"] = np.random.rand(
                *(1, num_kv_heads, 0, head_size)
            ).astype(np.float32)
    ...
    for _ in range(max_length):
        ...
        if sampling["method"] == "top_k":
            ...
            top_k_values, top_k_indices = (
                np.partition(logits[0], -top_k)[-top_k:],
                np.argpartition(logits[0], -top_k)[-top_k:],
            )
        ...
        if with_past:
            ...
            token_input_ids = np.concatenate(
                [token_input_ids, np.array([[next_token_id]], dtype=np.int64)], axis=-1
            )
            position_ids = np.arange(token_input_ids.shape[-1], dtype=np.int64).reshape(1, -1)
