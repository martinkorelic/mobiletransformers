...
from mobiletransformers.training.benchmark_datasets import DATASET_MAPPING, PEFTBenchmarkDataset
...
class CosineLRScheduler:
    ...
    def __repr__(self):
        return (
            f"CosineLRScheduler(warmup_steps={self.warmup_steps}, "
...
class ORTDataCurator:
    def __init__(
        ...
        self.prepare_dataset(
            ds, custom_preprocess=taskname_to_deepeval_preprocess_function(self.dataset_config.value)
        )
    ...
    def _setup_dataset(self, dataset_input):
        ...
        if isinstance(dataset_input, PEFTBenchmarkDataset):
            ...
            if dataset_input.lower() not in list(DATASET_MAPPING.keys()):
                raise ValueError(
                    f"Unsupported dataset: {dataset_input}. Choose from: {list(DATASET_MAPPING.keys())}"
                )
    ...
    def prepare_dataset(self, dataset: Dataset, custom_preprocess=None):
        ...
        if self.split:
            test_size = (
                self.test_ratio
                if self.max_dataset_length == None
                else int(self.max_dataset_length * self.test_ratio)
            )
            train_size = (
                (1 - self.test_ratio)
                if self.max_dataset_length == None
                else int(self.max_dataset_length * (1 - self.test_ratio))
            )
            dataset = dataset["train"].train_test_split(
                test_size=test_size, train_size=train_size, shuffle=self.shuffle
            )
...
class ORTTrainingArguments:
    ...
    def load_from_json(self, train_dir):
        with open(f"{train_dir}/training_config.json") as f:
            data = json.load(f)
...
class ORTTrainer:
    def __init__(
        ...
        if self.load_from_state:
            try:
                with open(f"{self.training_model_dir}/training_state.json") as f:
        ...
        self.data_curator = ORTDataCurator(
    ...
    def set_scheduler_type(self, total_steps):
        ...
        if self.args.scheduler_type == "linear":
            return LinearLRScheduler(
                self.optimizer, self.args.warmup_steps, total_steps, initial_lr=self.args.learning_rate
            )
    ...
    def load_onnx_trainer(self):
        ...
        self.model = Module(
            f"{self.training_model_dir}/training_model.onnx",
            self.checkpoint_state,
            f"{self.training_model_dir}/eval_model.onnx",
    ...
    def train(self):
        ...
        while global_step < total_steps:
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                position_ids = self.create_position_ids(
                    input_ids_np, padding_idx=self.data_curator.tokenizer.pad_token_id
                )
                ...
                if self.args.save_steps and (global_step + 1) % self.args.save_steps == 0:
                    self.save_checkpoint({"scheduler_state": self.scheduler.state_dict()})
        ...
        if self.args.export_inference:
            ...
            self.model.export_model_for_inferencing(
                f"{self.training_model_dir}/{self.inference_model_path}",
    ...
    def _create_model_name(self) -> str:
        ...
        model_short_name = (
            self.args.model_id.split("/")[-1] if "/" in self.args.model_id else self.args.model_id
        )
    ...
    def _load_peft_mapping(self):
        ...
        try:
            ...
            all_merger_files = [
                f for f in os.listdir(self.training_model_dir) if f.endswith("merger_model.onnx")
            ]
