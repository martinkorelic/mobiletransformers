...
from mobiletransformers.peft.adapters import load_mars_adapters
...
class CustomPeftModel(DeepEvalBaseLLM):
        ...
        if os.path.exists(base_tensors):
            ...
            if adapter_name == "base":
                ...
                raise FileNotFoundError(
                    "No 'adapter_config.json' or 'config.json' and their corresponding weights found!"
                )
        ...
        if is_adapter_model:
            ...
            if adapter_name in ["lora", "qlora", "loraq4", "loraq8"]:
                ...
                model = get_peft_model(
                    base_model, config, adapter_name="ablation", autocast_adapter_dtype=False
                )
                ...
                reconstr_config = {
                    ...
                    "r_squared": True,
                    "svd": {"rank": config.r, "n_iter": 10, "random_state": 42},
                }
                ...
                renamed_state_dict = {
                    ...
                    for (k, v) in peft_model_weights.items()
    ...
    def setup_attention_viz(
        self, max_new_tokens: int = 1, do_sample: bool = False, early_stopping: bool = True
    ):
        """
    ...
    def simple_attention_viz(
        ...
        if not (
            hasattr(self.generation_config, "output_attentions") and self.generation_config.output_attentions
        ):
        ...
        if average_heads:
            ...
            print("✓ Normalized attention weights (each row sums to 1)")
        else:
            # Single head
        ...
        # Title with results
        head_info = "Averaged across all heads" if average_heads else f"Head {head}"
        normalization_info = " (Normalized)" if average_heads else ""
        plt.title(
            f"Attention Pattern (Layer {layer}, {head_info}){normalization_info}\n"
            f'Generated: "{generated_text}"\n'
            f"Last {len(clean_tokens)} tokens",
            fontsize=12,
            pad=20,
        )
        ...
        if average_heads:
            print("📈 Averaged and normalized across all attention heads")
        else:
    ...
    def analyze_final_decision(
        ...
        ax1.set_title(
            f'What Influenced the Final Decision: "{generated_text}"\n'
            f"Top {len(top_influences)} Most Attended Tokens (Layer {layer})",
            fontsize=14,
            pad=20,
        )
        ...
        print(f"{'=' * 50}")
        print("📊 TOP INFLUENCES (what the model focused on):")
        ...
        question_tokens = [
            t
            for t, w in top_influences[:5]
            if any(
                keyword in t.lower() for keyword in ["france", "capital", "paris", "london", "berlin", "rome"]
            )
        ]
        if question_tokens:
            print(f"  ✓ Model focused on relevant tokens: {question_tokens}")
        else:
            print("  ⚠️  Model attention might be on structural tokens")

        print(f"📁 Detailed visualization saved to: {save_path}")

        return generated_text, final_attention, top_influences

    def analyze_prediction_probabilities(
        self, prompt: str, top_k: int = 20, save_path: str = "prediction_probs.png"
    ) -> dict:
        """
        ...
        ax1.set_title(
            f"Top {top_k} Token Predictions from Language Model Head\n"
