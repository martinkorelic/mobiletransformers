...
class ObjectBoxQueryEngine:
    def __init__(self, database_dir: str, model_name: str | None = None):
        ...
        self.store = Store(
            model=self.model,
            directory=str(self.database_dir),
            model_json_file=f"{self.database_dir}/objectbox-model.json",
        )
    ...
    def vector_similarity_search(
        ...
        for entity in all_entities:
            try:
                ...
                if len(entity.embedding) != len(query_embedding):
                    logger.warning(
                        f"Entity {entity.id} embedding dimension mismatch: {len(entity.embedding)} vs {len(query_embedding)}"
                    )
                    failed_comparisons += 1
                    continue
        ...
        logger.info(
            f"Similarity calculations: {successful_comparisons} successful, {failed_comparisons} failed"
        )
    ...
    def text_search(
        self, search_term: str, search_field: str = "content", case_sensitive: bool = False
    ) -> list[VectorSearchResult]:
        """
    ...
    def get_by_id(self, vector_id: int) -> VectorSearchResult | None:
        """
...
def main():
    ...
    parser.add_argument(
        "--database-dir", type=str, required=True, help="Directory containing the ObjectBox database"
    )
    ...
    parser.add_argument(
        "--min-score", type=float, default=0.0, help="Minimum similarity score (default: 0.0)"
    )
    ...
    parser.add_argument(
        "--list-all", action="store_true", help="List all vectors in database (for debugging)"
    )
    ...
    try:
        ...
        # Get by ID
            ...
            print(
                "Please specify a query type: --query, --text-search, --document, --vector-id, --list-documents, or --stats"
            )
            return
