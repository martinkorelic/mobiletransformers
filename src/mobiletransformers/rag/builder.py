...
from mobiletransformers.rag.vector_entity import (
    VectorEntity64,
    VectorEntity128,
    VectorEntity256,
    VectorEntity384,
...
class MobileTransformersObjectBoxProcessor:
        ...
        if not self.no_embed:
            ...
            if embedding_dim is None:
                ...
                if actual_dim != embedding_dim:
                    logger.warning(
                        f"Provided embedding dimension ({embedding_dim}) doesn't match model dimension ({actual_dim}). Using model dimension: {actual_dim}"
                    )
    ...
    def _get_entity_class(self, embedding_dim: int):
        ...
        if embedding_dim not in entity_map:
            raise ValueError(
                f"Unsupported embedding dimension: {embedding_dim}. "
    ...
    def load_documents(self, input_dir: Path) -> list[Document]:
        ...
        try:
            for json_file in input_dir.rglob("*.json"):
                try:
                    json_loader = JSONLoader(file_path=str(json_file), jq_schema=".", text_content=False)
        ...
        if splitter_type == "recursive":
            ...
            if markdown_headers:
                ...
                return MarkdownHeaderTextSplitter(
                    headers_to_split_on=headers_to_split_on, return_each_line=True, strip_headers=False
                )
            else:
    ...
    def create_vector_entity(
        self, name: str, content: str, document: str, embedding: list[float], metadata: dict[str, Any]
    ) -> Any:
    ...
    def process_and_store(
        self, documents: list[Document], text_splitter: Any, splitter_type, batch_size: int = 100
    ) -> int:
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            try:
                ...
                for j, (chunk, embedding) in enumerate(zip(batch_chunks, embeddings)):
                    ...
                    chunk_metadata = {"chunk_id": i + j, "chunk_index": j}
...
def process_documents_with_custom_entities(
    input_dir: Path,
    output_dir: Path,
    ...
    try:
        ...
        if splitter_type == "document":
            ...
            logger.info(
                f"Creating {splitter_type} text splitter (chunk_size={chunk_size}, overlap={chunk_overlap})"
            )
            if markdown_headers:
...
def validate_and_prepare_schema(script_dir: Path):
    ...
    try:
        with open(kotlin_schema_path) as f:
        ...
        if not vector_entities:
            logger.warning(
                "No VectorEntity classes found in schema. Expected VectorEntity64, VectorEntity384, etc."
            )
        else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Create ObjectBox vector database with custom entities using LangChain"
    )
    parser.add_argument(
        "--input-dir", type=str, required=True, help="Directory containing documents to process"
    )
    parser.add_argument(
        "--output-dir", type=str, required=True, help="Output directory for the ObjectBox database"
    )
    parser.add_argument(
        "--embedding-dim",
        type=int,
    ...
    parser.add_argument("--chunk-size", type=int, default=128, help="Text chunk size (default: 128)")
    parser.add_argument("--chunk-overlap", type=int, default=32, help="Chunk overlap size (default: 32)")
    parser.add_argument(
        "--splitter-type",
        type=str,
        default="recursive",
    ...
    parser.add_argument(
        "--no-embed", action="store_true", help="Skip embedding generation and store empty vectors"
    )
    parser.add_argument(
        "--markdown-headers",
        type=str,
    ...
    logger.info(
        f"Embedding dimension: {'auto-infer from model' if args.embedding_dim is None else args.embedding_dim}"
    )
