...
/**
 * TODO: Implement own tokenizer without using
 *
 */
class ORTGenAiTokenizer(folderPath: String) {
    ...
    fun generate(promptText: String) {
        ...
        //generator.getLastTokenInSequence()
        //val generatorParams = model.createGeneratorParams()
        //generatorParams.setInput(tokens)

        //val output_ids = model.generate(generatorParams)
        //val result = tokenizer.decode(output_ids)


        //val generatorParams: GeneratorParams = model.createGeneratorParams(promptText)

        //val fullResponse: String = model.generate(generatorParams, null)

        //Log.d("ORTGenerator", "${output_ids}")
