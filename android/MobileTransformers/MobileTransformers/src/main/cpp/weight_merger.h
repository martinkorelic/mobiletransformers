...
// Was reached transitively via weight_serializer.h until that dead TensorProto layer was deleted (#23).
...
#include "constants/merger_variant.h"  // #6: typed MergerVariant (parity-checked wire values)
...
private:
    ...
    // Keyed by the TYPED variant (#6), not by a raw tag string.
    std::unordered_map<MergerVariant, std::unique_ptr<Ort::Session>> merger_sessions_;
    ...
    // Resolve this layer's merger variant from adapter shape + quantization (nullopt = none applies).
    ...
    // Returns false if any role of any layer could not be written (#9 fail-closed).
    bool save_merged_parameters(const std::string& output_directory);
