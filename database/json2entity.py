        ...
        for prop in entity.get('properties', []):
            ...
            if prop_name == 'id':
                ...
                if 'indexId' in prop:
                    ...
                    python_code.append(f"    {prop_name} = String(id={prop_id}, uid={prop_uid}, index=Index(type=IndexType.VALUE, uid={index_uid}))")
                else:
                    python_code.append(f"    {prop_name} = String(id={prop_id}, uid={prop_uid}, index=Index(type=IndexType.VALUE))")
            else:
    ...
    for entity in data.get('entities', []):
        ...
        for prop in entity.get('properties', []):
            ...
            if prop_name == 'id':
                ...
                kotlin_annotations.append(f"    @HnswIndex(dimensions = {dimensions}, distanceType = VectorDistanceType.COSINE)")
                kotlin_annotations.append(f"    @Uid({prop_uid}) override var {prop_name}: FloatArray = floatArrayOf(),")
            elif prop_name == 'content':
                # Always add @Index for content field
                kotlin_annotations.append(f"    @Index @Uid({prop_uid}) override var {prop_name}: String = \"\",")
            else:
...
def create_filtered_json_model(json_file_path, entity_names_to_include, output_path="objectbox-model/default.json"):
    """
