...
**Priority #22 | Prerequisites: #10 (`01_code_plans/03_inference_engine_abstraction_native_and_genai.md`), #8 (`01_code_plans/01_unified_merger_and_external_data_export.md`), #7 (`00_code_plans/07_weight_handoff_map_and_tensor_codec.md`) | Blocks: #23 (`03_code_plans/02`), #25 (`03_code_plans/04`)**

## Purpose

Make the Native inference engine a deliberate, documented, fail-closed component sitting behind the already-defined `ModelRuntime` boundary (#10). This plan does **not** define an engine interface — that is owned by #10. It (a) wires `ORTGeneratorNative` into `ModelRuntime`, (b) retires the legacy `inference/merged/` handoff in favor of the external-initializer + `weight_handoff_map.json` contract (#7/#8), (c) documents the real graph I/O contract, and (d) fixes the conversation-state prepend bug and adds train↔generate lifecycle tests.
...
- `android/.../repository/LLMRepository.kt` — `makeOrtNativeInference()` (`:262-282`) and `prepareGeneration` (`:334-380`) route through `ModelRuntimeFactory` (#10), not a direct `ORTGeneratorNative(...)`.
- `android/.../ORTGenAINative.kt` — **delete the six dead `NotImplementedError` stubs** (`:74-94`: `releaseWeightSession`/`releaseGenAISession`/`initializeGenAIInference`/`performGenAIInferenceStep`/`cacheSessionWeights`/`createGenAISession`). This abandoned JNI-GenAI bridge is superseded by the real GenAI engine (`ORTGeneratorGenAI`, owned by #10); it must not linger as half-implemented surface. Retire the file (and `onnx-genai.cpp`) as part of this cleanup.
...
- `android/.../cpp/session_cache.h` — the external-initializer load path (`session_cache.h:662-709`, `AddExternalInitializers` at `:702`) is the handoff consumer; gains the map-driven naming from #7 (do not re-derive names from `<dirname>.<filestem>`).
...
→ exposed as RuntimeCapabilities.supportsLoadMergedWeights (#10)
```
...
3. **Map-driven load** in `session_cache.h:662-709`: take initializer names from `inferenceInitializerNames[role]` (#7), not filesystem reconstruction; verify dtype/shape; then `AddExternalInitializers`.
...
- **#10 (`ModelRuntime`)**: this plan supplies the Native impl; #10 owns selection/fallback/parity.
- **#7/#8 (handoff map / merger)**: this plan is the load-side consumer; names/dtypes/order come from there.
- **#23 (`03_code_plans/02`)**: depends on the hardened `generate(...)` for the sampling/streaming public-name mapping.
- **`05_code_plans/04`**: the documented I/O contract becomes `docs/ARCHITECTURE.md` content.

## Tests & smokes

- **Handoff precondition unit test (JVM)**: map present + all files → ready; missing file / bad checksum / wrong dtype / wrong shape → not ready and a fail-closed error naming the tensor.
- **Native graph I/O smoke (Android)**: load a tiny fixture, assert required inputs present, generate one token.
- **Conversation reset test**: two sequential prompts; assert no token from prompt 1 leaks into prompt 2.
- **Train↔generate lifecycle test**: open training session → close → open generation session over the same cache → generate; assert no session/handle leak and merged weights are reflected.
- **No `merged/` regression**: grep the build output / runtime paths; assert nothing constructs or reads `inference/merged/`.
- **No dead-stub regression**: grep for `NotImplementedError` in `ORTGenAINative`; assert the file (and `onnx-genai.cpp`) are gone, not left half-implemented.
