...
**Priority #20 | Prerequisites: #12 (`00_code_plans/06_manifest_first_package_and_cache_bridge.md`), #13 (`02_code_plans/03_hub_model_package_format.md`) | Blocks: sample-app facade load (#18), starter-zoo loading**
...
- `mobiletransformers/hub/pull.py` — `pull_package()` + `install_package()`; the `pull` / `install-package` CLI subcommands (registered alongside `export`/`push` from #14).
...
- Reuses `mobiletransformers/hub/package_format.py` (#13): `FEATURE_GROUPS`, `sanitize_repo_id`, `build_manifest` round-trip helpers.
...
- Reuses #12's cache-bridge mapper (variant subtree → cache layout).

## Data contracts / interfaces

### Cache layout (target, owned by #12; restated)
...
> Mapping note: the Hub repo splits tokenizer under `shared/tokenizer/` and chat template under `shared/chat_template.jinja`; the cache bridge flattens these into the single `<repo>/tokenizer/` dir the repository expects. `inference/base` + `inference/merged` are kept as subdirs (the engine + #8/01 merge contract read them).
...
2. `install_package(staging, cache_root, repo_id)`:
   - Call #12's cache-bridge mapper to transform `variants/<variant>/...` + `shared/...` → `<cache_root>/<sanitize_repo_id(repo_id)>/{train,inference,embedding,tokenizer}` + copy `mobiletransformers_manifest.json` + `checksums.json`.
...
7. On all-files-complete: run the Kotlin cache-bridge mapper (#12) inside the worker to reshape `.partial` into the final cache layout, then atomic `File.renameTo` (same filesystem) of `.partial/<sanitizedRepoId>` → `<cacheDir>/<sanitizedRepoId>`. Write `mobiletransformers_manifest.json` + `checksums.json` last.
...
10. Surface as `MobileTransformers.fromPretrained(context, repoId, revision, variant?, features, cacheDir)` (facade in #18) → enqueue worker → await → return `MobileTransformerModel`.

## Interactions

- **#13 (package format):** source of `downloadPlan`, `sha256`, `fileSizes`, `etag`, `variants`, `defaultVariant`; `sanitize_repo_id` parity.
- **#12 (cache bridge):** owns the variant-subtree → cache-layout mapper used by both `install_package` (Python) and `PackageDownloadWorker` (Kotlin), plus the manifest validator run before exposing the package.
- **#14 (export CLI):** produces the package this flow consumes; the tiny fixture from #13 backs the smokes here.
- **#18 (Kotlin facade):** `fromPretrained` is the public entry that drives the Android downloader.
...
Python:
- `select_variant` unit table: device-constraint rows → expected variant id, incl. fallback-to-default and storage-exceeded failure.
...
- `sanitize_repo_id` Kotlin↔Python parity via the shared `sanitize_repo_id_cases.json` (from #13).
