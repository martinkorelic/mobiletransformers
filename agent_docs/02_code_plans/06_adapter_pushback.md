...
**Priority #21 (last) | Prerequisites: #8 (`01_code_plans/01_unified_merger_and_external_data_export.md`), #13 (`02_code_plans/03_hub_model_package_format.md`) | Blocks: nothing (terminal)**
...
New (Python pkg root; reuses #14's `model_card.py` + push path):
...
- `mobiletransformers/adapter/model_card.py` — thin wrapper over `mobiletransformers/export/model_card.py` (#14) adding adapter-specific sections.
...
Reads from the on-device cache shape (produced by #20, owned by #12):
...
- `<cacheDir>/<sanitizedRepoId>/train/weight_handoff_map.json` — train→infer tensor contract (#7/07).
...
2. **MobileTransformers-native adapter (gate fallback, esp. MARS):** emit the per-tensor merged initializers + `weight_handoff_map.json` + `training_config.json` under a `variants/<id>/train/` style subtree (#13), with the model card stating it is **not** a drop-in PEFT adapter and must be re-applied via MobileTransformers.
...
4. `model_card`: call `render_model_card` (#14) extended with the adapter sections above; assert privacy + license blocks present (fail upload if missing).
...
- **#8 (unified merger + external-data):** defines the `inference/merged/` per-tensor files and the merge semantics this exporter reads; the MARS-vs-PEFT gate hinges on what #8's handoff map records.
- **#13 (package format):** mode-2 native adapters reuse the `variants/<id>/train/` subtree shape and the `weight_handoff_map.json` location.
- **#7/07 (handoff map):** authoritative source for `handoffMode` and per-tensor name mapping; the gate decision reads its flags.
- **#14 (export CLI):** shares `model_card.py` and the `huggingface_hub.upload_folder` push path.
...
- `to_peft_layout` gate: LoRA fixture → returns `PeftAdapter` with valid `adapter_config.json`; MARS-opt4 (quantized) fixture → returns `None` and the CLI falls back to mode 2.
...
- Round-trip (optional, gated): export PEFT adapter → reload with `PeftModel.from_pretrained` (already used in `inference/builder.py`) to confirm the layout loads.
