...
Collapse the current multi-script export flow (`inference/builder.py create_model` → `artifact/onnx_builder.py convert_pipeline` → manual config/tokenizer/merger emission) into **one command** that turns a HuggingFace repo id into a MobileTransformers-ready package directory (the #14 layout) plus `mobiletransformers_manifest.json`, `checksums.json`, `weight_handoff_map.json`, and `optimum/export_report.json`.
...
- `mobiletransformers/cli/main.py` — the **existing** argparse dispatcher from `00_code_plans/01` (console script `mobiletransformers = mobiletransformers.cli.main:main`, already in `pyproject.toml`). This plan registers subcommands `export`, `push` (and #21 adds `pull`, `install-package`) via plan 01's `add_parser`/`run` contract — no new `__main__.py`, no typer.
...
- `inference/model.onnx` + `inference/frozen_base.onnx.data` (frozen quantized base, immutable) + per-tensor trainable `<name>.bin` (+`.sha256`) **flat** in `inference/` (#9/01 split — no `base/` or `merged/` subdirs).
...
1. **Load config + settings.** Read `config/config.yml` defaults (sections via `mobiletransformers.config.constants`), overlay CLI flags through the #4 settings layering. Resolve the HF token via `get_settings().require_hf_token()`. Build the effective `TRAIN_BUILDER` / `INFERENCE_BUILDER` / `ARTIFACT_BUILDER` dicts.
...
4. **Normalize.** Run the #9 external-data normalization: force per-tensor external initializers for trainable MatMul weights (today `force_dequantize_external_and_save()` in `artifact/onnx_builder.py` does the forcing) and split into the frozen `frozen_base.onnx.data` blob + trainable per-tensor `.bin` files **flat in `inference/`** (the legacy `inference/merged/` subdir is retired per the canonical external-initializer decision). Normalize KV-cache names and trainable-weight initializer names to match the engine's expectations. Emit `weight_handoff_map.json` (#8/07) recording the train-checkpoint-name → external-initializer-file mapping.
...
11. **Optional desktop smoke (`--validate`).** Run `convert_pipeline`'s training check (`onnx_checktrain`, one train step + optimizer step) and a 1-token inference generation against `inference/model.onnx`. Record pass/fail into `export_report.json`. Smoke runs in the ORT-training profile; generation may run in a separate profile if package conflicts (matches the existing comment in `convert_pipeline` that skips generation testing in the artifact script).
...
- **#9 (unified merger + external-data):** owns the frozen-blob + flat per-tensor external-initializer split and the normalization (step 4) and informs the handoff map (#8/07).
...
- Handoff-map test: every entry's `externalDataLocation` resolves to a real file in the variant's `inference/` dir.
