...
- **#7 (`optimum_onnx_export_and_tasksmanager`):** the source of the `TasksManager` wrapper, task-priority logic, and `optimum/*.json` reports. This plan reuses #7's task selection rather than duplicating it; keep `_select_task` identical to #7's chooser.
