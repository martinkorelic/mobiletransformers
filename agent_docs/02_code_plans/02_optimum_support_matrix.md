...
**Priority #20 | Prerequisites: #7 (`01_code_plans/05_optimum_onnx_export_and_tasksmanager.md`) | Blocks: starter-zoo docs, contributor model-onboarding**

## Purpose

Produce a single machine-readable `model_support_matrix.json` that records, per candidate model, how far it gets along the MobileTransformers readiness pipeline — from "Optimum can export it" all the way to "Android can train it and RAG works." The matrix is the reporting layer over #7's `TasksManager`-based candidate detection. It uses **inherited statuses** (a later status can only be `true` if its predecessor is `true`) plus a `blockers[]` list so contributors can see exactly where a newly Optimum-supported architecture stalls. Only the *ready* statuses (`android_inference_ready`, `android_training_ready`, `rag_ready`) surface in user-facing starter-zoo docs; the earlier statuses are contributor-only.

This aligns MobileTransformers to Optimum ONNX as the first support filter (decision in `02_tier1_hf_integrated_core.md` §"Optimum ONNX Support Alignment Plan") rather than maintaining a hand-curated list. Today there is **no** support-matrix generation in the repo — PEFT target modules are hard-coded in `peft_models/mars/utils.py` (`TRANSFORMERS_MODELS_TO_MARS_TARGET_MODULES_MAPPING`; by the time this plan runs, `00_code_plans/09` has absorbed that table into the architecture registry), and `trainer/builder.py` imports concrete Optimum configs (`LlamaOnnxConfig, Qwen2OnnxConfig, Phi3OnnxConfig, …`) without querying `TasksManager`. This plan introduces the discovery + reporting layer.

**Verified facts this plan relies on:** optimum-onnx 0.1.0 exposes `optimum.exporters.tasks.TasksManager.get_supported_tasks_for_model_type(model_type, "onnx")`; transformers ceiling is `<4.58`. (Note: the repo's current `requirements-*.txt` pin `optimum==1.23.x` — the legacy bundled package. #2/#7 migrate to standalone `optimum-onnx`; this plan targets the post-migration API and records the resolved versions at runtime rather than hard-coding them.)
...
- CLI wiring (#15 `one_command_export_cli`): add a `mobiletransformers support-matrix` subcommand that calls `build_matrix`.

Reused from #7: the `TasksManager` wrapper, the task-priority selection, and the export-report it emits (`optimum/export_report.json`, `optimum/supported_tasks.json`). This plan consumes those rather than re-querying when an export already ran.
...
| `mobile_package_exportable` | dry-run of #7/#15 normalization succeeds (tokenizer via `AutoTokenizer`, `generation_config.json`, manifest writable); RAG models need `feature-extraction` |
| `train_artifacts_exportable` | ORT training-artifact generation succeeds and PEFT/MARS target modules resolve — checked against the **architecture registry** (`ARCHITECTURE_REGISTRY[arch].target_modules`, `00_code_plans/09` — which absorbed the legacy `TRANSFORMERS_MODELS_TO_MARS_TARGET_MODULES_MAPPING`); unknown architecture ⇒ blocker "MARS target module mapping not verified" |
...
**Schema versioning (F1).** The envelope carries `schemaVersion` (`"MAJOR.MINOR"`) plus `minReaderVersion`; readers preserve unknown fields so additive minor bumps (new per-entry keys, new statuses) stay non-breaking, and fail closed with a "needs newer SDK" message only when the matrix's `major` exceeds support or `minReaderVersion` is unmet. The same `check_compat()` helper is mirrored Python↔Kotlin so any consumer of this JSON gates identically.

**Single source of truth (F6).** `model_support_matrix.json` is the *generated* truth — `docs/COMPATIBILITY_MATRIX.md` (and any starter-zoo table) is rendered FROM it, never hand-edited, and each Hub package's `mobiletransformers_manifest.json` declares only *that* package's realized capabilities. Do not hand-maintain three lists: the generator emits the matrix, the doc render reads it, and per-package manifests restate only their own slice.
...
3. **`mobile_package_exportable`.** If a real export already ran (#7/#15 produced `optimum/export_report.json` + `optimum/supported_tasks.json` for this model), trust it. Otherwise dry-run the normalization checks (tokenizer load via `AutoTokenizer`, generation config availability, manifest writability) and set the flag.
4. **`train_artifacts_exportable`.** Check ORT training-artifact generation availability for the selected task and resolve PEFT/MARS target modules via `09.resolve_architecture(config).target_modules` (the architecture registry is authoritative — this plan lands at order #20, after 09 collapsed the `peft_models/mars/utils.py` table into it); set `marsTargetModulesKnown` and, if the architecture is not registered, add blocker "MARS target module mapping not verified for model_type '<type>'" and force the status `false`.
...
8. **CLI.** Add `mobiletransformers support-matrix --config config.yml --out build/support/model_support_matrix.json [--docs docs/model_support_matrix.json] [--probes build/support/android_probes.json]` to the #15 CLI.

## Interactions

- **#7 (`optimum_onnx_export_and_tasksmanager`):** the source of the `TasksManager` wrapper, task-priority logic, and `optimum/*.json` reports. This plan reuses #7's task selection rather than duplicating it: `_select_task` is a thin import/alias of `mobiletransformers/export/registry.py::choose_task` (#7) — do NOT copy the priority list into this module.
- **#15 (one-command export CLI):** hosts the `support-matrix` subcommand; an export run can update a single model's entry in place.
- **`00_code_plans/09` architecture registry:** authoritative source for `train_artifacts_exportable`'s MARS-mapping check (it absorbed `peft_models/mars/utils.py`'s table) — when a new architecture registry entry lands, the matrix flips `marsTargetModulesKnown` to true on regeneration.
...
- **Hub package (#14) / docs:** user-facing starter-zoo docs render only the filtered matrix.

## Tests & acceptance

**Unit (automated)** — small, fast; prove the component wires together and compiles.
- **Inheritance unit test** (`pytest tests/support/test_inheritance.py`): `{optimum_exportable:true, mobile_package_exportable:false, train_artifacts_exportable:true,...}` → after `apply_inheritance`, everything from `mobile_package_exportable` onward is `false`; `first_blocked` returns `mobile_package_exportable`.
...
- **MARS-mapping test:** an architecture present in `ARCHITECTURE_REGISTRY` with `target_modules` → `marsTargetModulesKnown=true`; an unregistered one → `train_artifacts_exportable=false` + the specific blocker string.
...
- **Schema-version test (F1):** envelope carries `schemaVersion="1.0"` + `minReaderVersion`; `check_compat()` accepts an unknown extra field (additive minor) and fails closed with a "needs newer SDK" message when `major` exceeds support or `minReaderVersion` is unmet.
...
- **Live detection over real candidates:** run `support-matrix` un-mocked against the real `config.yml` family list (network + `AutoConfig.from_pretrained` downloads, plus optional `trust_remote_code` models) to confirm `TasksManager` returns tasks for each and no candidate crashes detection.

**Definition of done** — `mobiletransformers support-matrix` emits a `model_support_matrix.json` (envelope with `schemaVersion`/`minReaderVersion`, `statusOrder`, `userFacingStatuses`, per-model entries) whose inherited statuses never outrank a failed predecessor and where every earliest `false` has a blocker; the filtered user-facing JSON contains only `USER_FACING_STATUSES`-true models; the doc table renders FROM this JSON (not hand-edited); and all unit/integration tests above pass network-mocked.
