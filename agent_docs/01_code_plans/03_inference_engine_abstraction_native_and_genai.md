...
**Priority #10 | Prerequisites: #9 (`01_code_plans/02_genai_external_data_swap_spike.md`, which feeds Gate 0.1) | Blocks: #18 (`02_code_plans/01_hf_style_kotlin_facade.md`)**

## Purpose

Introduce a single `ModelRuntime` engine interface with two implementations — **Native** (default, guaranteed) and **GenAI** (opt-in, gated on Gate 0.1) — both operating over the **same package / same `inference/` folder** produced by File #8. The engine is a *selection over one package*, never a separate package or build.
...
This wraps the existing `ORTGeneratorNative` (Native) and the new GenAI JNI wrapper seeded by File #9's `genai_spike.cpp` (replacing the commented-out `onnx-genai.cpp` and the dead `ORTGenAINative`).
...
C++:
- NEW `android/.../cpp/genai_runtime.cpp` — promote `genai_spike.cpp` (File #9) to a real wrapper: `OgaCreateModel(<inference dir>)`, tokenizer, generator loop with token-by-token streaming, sampling config, release. Uses stable APIs only (no `OgaCreateModelWithInitializers`).
...
- The `loadMergedWeights` precondition (File #8) is "handoff map present + all `external_file`s exist with valid checksums" — checked once and exposed via `RuntimeCapabilities.supportsLoadMergedWeights`.
...
`genaiAvailable()` = Gate 0.1 passed AND the GenAI `.so` is linked AND `OgaCreateModel` symbol present (the File #9 symbol check, run once at init). Never throw to the caller for a GenAI-load failure; fall back.

## Implementation steps

1. **Define `ModelRuntime` + `Engine` + `RuntimeCapabilities`** in `ModelRuntime.kt`.
2. **Adapt Native**: make `ORTGeneratorNative` implement `ModelRuntime`. Factor its session open into `load(...)`, keep `generate(...)` and the callback emission as-is. Populate `capabilities` with `engine = NATIVE`, streaming true. Move the `inference/merged` probe (lines 41-48) to the new "handoff map + checksums" check from File #8.
...
7. **`genaiAvailable()` init check**: run the File #9 symbol check once (cache result); also verify the Gate 0.1 flag.
...
10. **Wire the facade**: File #18 (`02_code_plans/01`) calls only `ModelRuntime`, never a concrete engine.

## Interactions

- **File #9 / Gate 0.1**: gates whether GenAI is ever selected; provides the symbol check and the `genai_runtime.cpp` seed.
- **File #8**: provides the single `inference/` folder + handoff map + checksums that both engines consume; defines the `loadMergedWeights` precondition.
- **`00_code_plans/06` (manifest)**: declares `supported_engines` / `default_engine`; this plan reads them.
- **File #18 (HF-style facade)**: `fromPretrained`/generate route through `ModelRuntime`; `ModelFeature.GenAI` / `ModelFeature.ManualInference` map to `Engine.GENAI` / `Engine.NATIVE` as selectors, not as separate downloads.
...
- **Interface conformance**: both `ORTGeneratorNative` and `ORTGeneratorGenAI` satisfy `ModelRuntime`; `capabilities.engine` correct.
- **Same-folder dual-engine smoke**: load the same File #8 `inference/` dir under each engine, generate one token, both succeed (this is the runtime expression of Gate 0.1 equivalence).
- **Streaming parity smoke**: identical ordered callback events (`onStartGeneration` -> `onPartialResult`* -> `onCompletion`) for a greedy fixed prompt across both engines; identical token sequence at temp 0.
- **Fallback smoke**: force GenAI load failure (corrupt `genai_config.json` / missing symbol) and assert the factory returns Native and the caller sees no error, only a logged warning.
- **`genaiAvailable()` smoke**: with the symbol absent (Gate 0.1 fail simulation), GenAI is never offered even if `config.engine = GENAI`.
- **Manifest selection smoke**: a variant declaring `supported_engines: ["native"]` rejects/ignores a GENAI request and uses Native.
- **Merged-weights reflected per engine**: after train->merge (File #8), both engines reflect the changed output on reload.
- **Dead-code removal regression**: project builds with `ORTGenAINative.kt` and `onnx-genai.cpp` removed; no dangling references in `CMakeLists.txt` or imports.
